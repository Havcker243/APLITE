...
def verify_tatin_id(
    ...
    """Verify a Tatin ID (e.g. bulldogbites@tatin). No auth required."""
    ...
    # Accept both "bulldogbites@tatin" and bare "bulldogbites"
    if "@" in raw:
    ...
    handle = handle.strip()
    if not handle:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid Tatin ID")
