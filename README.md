...
TATIM is a verified payment identity system for businesses. It replaces raw bank-detail sharing with short TATIM IDs that can be verified publicly and resolved to payout coordinates by authorized partners.

Example ID: `bulldogbites@tatim`
...
- Organization and child TATIM ID issuance
...
6. Approved organizations receive an organization TATIM ID.
7. Verified users create payment accounts and child TATIM IDs.
...
Do not use `aplite-backend/data/schema.sql` for a fresh environment; it is an older draft and is missing onboarding fields used by the application.
...
| `UPI_SECRET_KEY` | Yes | HMAC key for TATIM ID generation |
...
Rate-limit variables are available for global traffic, resolve, lookup, onboarding uploads, account creation, child-ID creation, and public directory endpoints. See `aplite-backend/.env.example` for the full list.
...
- Approved users can create child TATIM IDs and resolve payment details.
...
- `GET /api/public/verify?id={tatim_id}`
...
- service name: `tatim-backend`
- root directory: `aplite-backend`
...
2. Set the project root directory to `aplite-frontend`.
...
8. Verified users can create payment accounts and child TATIM IDs.
...
- `aplite-backend/data/schema.sql` remains in the repo for historical reference only.
- Uploaded sample files under `aplite-backend/data/uploads/` are local artifacts, not required for a fresh environment.
