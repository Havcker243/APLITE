...
import { verifyTatinId, VerifyTatinIdResult } from "../utils/api";
...
export default function VerifyPage() {
  ...
  const [result, setResult] = useState<VerifyTatinIdResult | null>(null);
  ...
  async function runLookup(id: string) {
    ...
    setSearched(trimmed);
    try {
      const data = await verifyTatinId(trimmed);
      setResult(data);
    } catch {
            ...
            <div className="text-center py-16">
              ...
              <div className="flex gap-3 justify-center mt-4 text-sm">
                <button
                  ...
                  onClick={() => { setInputId("bulldogbites@tatin"); void runLookup("bulldogbites@tatin"); }}
                >
                  Try bulldogbites@tatin
                </button>
                <span className="text-muted-foreground">·</span>
                <button
                  type="button"
                  className="font-mono text-primary underline underline-offset-2 hover:text-primary/80"
                  onClick={() => { setInputId("fastfreight@tatin"); void runLookup("fastfreight@tatin"); }}
                >
                  Try fastfreight@tatin
                </button>
              </div>
            </div>
          )}
...
function VerifiedCard({ result, searched }: { result: VerifyTatinIdResult; searched: string | null }) {
  return (
    ...
    <div className="bg-card border border-destructive/30 rounded-xl p-8 shadow-card animate-fade-in">
      ...
      {searched && (
        <div className="bg-muted/50 rounded-lg px-4 py-3 mb-4 font-mono text-sm text-muted-foreground">
          {searched.includes("@") ? searched : `${searched}@tatin`}
        </div>
      )}
