...
export default function OnboardStep6() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = window.localStorage.getItem(`tatin_call_scheduled:${userId}`);
  ...
  useEffect(() => {
    ...
    if (callScheduled) {
      try {
        window.localStorage.setItem(`tatin_call_scheduled:${userId}`, "1");
      } catch {
        // ignore storage errors
      }
      markStepComplete(6);
    ...
    try {
      window.localStorage.removeItem(`tatin_call_scheduled:${userId}`);
    } catch {
      // ignore storage errors
    }
