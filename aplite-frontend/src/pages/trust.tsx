...
export default function TrustPage() {
  /** Render the trust page content. */
  const { token } = useAuth();
  const backHref = token ? "/dashboard" : "/";
  return (
    <div className="min-h-screen bg-gradient-subtle relative overflow-hidden">
      <Head>
        <title>Trust and Transparency - TATIM</title>
      </Head>
      ...
      <main className="mx-auto w-full max-w-5xl px-6 py-12">
        <div className="mb-10 rounded-lg border border-border bg-background/80 p-8 shadow-card backdrop-blur">
          ...
          <p className="mt-3 text-sm text-muted-foreground leading-relaxed">
            TATIM provides a verified payment identity system that gives Accounts Payable teams confidence in vendor
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <section className="rounded-lg border border-border bg-card p-6 shadow-card">
            <h2 className="text-base font-semibold text-foreground">How TATIM works</h2>
            <p className="mt-3 text-sm text-muted-foreground leading-relaxed">
