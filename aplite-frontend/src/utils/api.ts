...
export type VerifyTatinIdResult = {
  verified: boolean;
  handle: string;
  name: string | null;
  industry: string | null;
...
export async function verifyTatinId(tatinId: string): Promise<VerifyTatinIdResult> {
  const params = new URLSearchParams({ id: tatinId });
...
export async function exportAccountData(): Promise<void> {
  ...
  a.href = url;
  a.download = "tatin-data-export.json";
  a.click();
